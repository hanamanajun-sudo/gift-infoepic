...
for b in r.json().get('results', []):
    ids.append(b['id'])
print(f"Deleting {len(ids)} blocks")
for bid in ids:
...
B.append(bp([("친구·또래 (1~3만원): ",True),"용돈 범위에서 '센스 있는' 느낌이 중요. 다꾸 스티커, 립 제품, 폰 악세서리류가 1~2만원대면 포장까지 예쁘게 가능해요."]))
...
B.append({"object":"block","type":"table","table":{"table_width":4,"has_column_header":True,"has_row_header":False,"children":[
    tr(["예산","추천 아이템","가격대","누가?"]),
